...
class Simulator:
    ...
    def save_to_disk():
        ...
        rtset = results_file.create_dataset(f"time", self.grid.shape, dtype='f', data=self.grid)
        rset = results_file.create_dataset(f"{self.title}", self.results.shape, dtype='f', data=self.results)
        results_file.close()
