...
gpuForcesParallelo(double3 *forces, double3 *positions, double epsilon, double sig, int N){
    ...
    dvec = positions[idy] - positions[idx];
    ...
    double3 newforce = result*dvec;
    atomicAdd( reinterpret_cast<double*>(forces+idx)+0, newforce.x);
    atomicAdd( reinterpret_cast<double*>(forces+idx)+1, newforce.y);
    atomicAdd( reinterpret_cast<double*>(forces+idx)+2, newforce.z);
...
for i in range(1):
    ...
    X = np.array(
        [[0, 0, 0], [1, 0, 0], [3, -2, 0], [11, -2, 3]], dtype=np.float64, order="C"
    )
